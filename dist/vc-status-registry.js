...
class VcStatusRegistry {
    ...
    getPastStatusSetEvents(did, fromBlock = 0, toBlock = 'latest') {
        const filter = {
            ...
            fromBlock: fromBlock,
            toBlock: toBlock,
            topics: [ethers_1.ethers.utils.id('VcStatusSet(address,address)'), ethers_1.ethers.utils.id(did)]
        };
    ...
    getPastStatusRemoveEvents(did, fromBlock = 0, toBlock = 'latest') {
        const filter = {
            ...
            fromBlock: fromBlock,
            toBlock: toBlock,
            topics: [ethers_1.ethers.utils.id('VcStatusRemoved(address,address)'), ethers_1.ethers.utils.id(did)]
        };
        return this.provider.getLogs(filter);
    }
}
